...
class DatabaseCoreManager:
    ...
    def _build_sql_params_for_where(  # noqa: PLR6301
        ...
        for column, condition_obj in where_clauses.items():
            ...
            elif isinstance(condition_obj, tuple) and len(condition_obj) == NoYes.YES:
    ...
    def execute_query(self, **kwargs) -> dict[str, Any]:  # noqa: PLR0912, PLR0914
        """
        ...
        if self.schema:
            from_clause = f'FROM {self.schema}.{table}'
        else:
